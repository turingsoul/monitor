...
public class HibernateUtil implements IPentahoSystemEntryPoint, IPentahoSystemExitPoint {
    ...
    protected static boolean initialize() {
        ...
        String hibernateManagedString = lookupSetting( applicationContext, "hibernateManaged",
                "settings/managed",
        ...
        try {
            ...
            if ( hibernateConfigurationFile != null ) {
                String configPath = applicationContext.getSolutionPath( "system/" + hibernateConfigurationFile );
                File cfgFile = new File( configPath );
                if ( cfgFile.exists() ) {
                    HibernateUtil.configuration.configure( cfgFile );
                } else {
                    HibernateUtil.log.error( "HIBUTIL.ERROR_0012_CONFIG_NOT_FOUND" + configPath );
                    return false;
                }
            } else {
                // Assume defaults which means we hope Hibernate finds a configuration
                // file in a file named hibernate.cfg.xml
                HibernateUtil.log.error( "HIBUTIL.ERROR_0420_CONFIGURATION_ERROR_NO_HIB_CFG_FILE_SETTING" );
            ...
            if ( !HibernateUtil.hibernateManaged ) {
                HibernateUtil.log.info( "HIBUTIL.USER_HIBERNATEUNMANAGED" );
                ...
                if ( HibernateUtil.factoryJndiName == null ) {
                    HibernateUtil.log.error( "HIBUTIL.ERROR_0013_NO_SESSION_FACTORY" );
                    return false;
                }
                HibernateUtil.log.info( "HIBUTIL.USER_HIBERNATEMANAGED" );
            ...
            HibernateUtil.log.error( "HIBUTIL.ERROR_0006_BUILD_SESSION_FACTORY", ex ); //$NON-NLS-1$
            throw new ExceptionInInitializerError( ex );
        }
    }

    private static IDBDatasourceService getDatasourceService() throws ObjectFactoryException {
        return PentahoSystem.getObjectFactory().get( IDBDatasourceService.class, null );
    }
    ...
    private static String lookupSetting( IApplicationContext applicationContext, String applicationContextName,
        ...
        // 2- Look in system/hibernate-settings.xml
        // 3- Look in monitor.xml
        ...
        if ( !HibernateUtil.hibernateManaged ) {
            synchronized ( HibernateUtil.lock ) {
                try {
                    ...
                    throw new RepositoryException( "HIBUTIL.ERROR_0007_REBUILD_SESSION_FACTORY", ex ); //$NON-NLS-1$
                }
            }
        } else {
            try {
        ...
        try {
            if ( s == null ) {
                if ( HibernateUtil.debug ) {
                    HibernateUtil.log.debug( "HIBUTIL.DEBUG_OPEN_NEW_SESSION" ); //$NON-NLS-1$
                }
                if ( HibernateUtil.getInterceptor() != null ) {
                    if ( HibernateUtil.debug ) {
                        HibernateUtil.log
                                .debug( "HIBUTIL.DEBUG_USING_INTERCEPTOR" + HibernateUtil.getInterceptor().getClass() );
                    }
            ...
            HibernateUtil.log.error( "HIBUTIL.ERROR_0005_GET_SESSION", ex ); //$NON-NLS-1$
            throw new RepositoryException( "HIBUTIL.ERROR_0005_GET_SESSION", ex ); //$NON-NLS-1$
        }
        return s;
    }
    ...
    public static void closeSession() throws RepositoryException {
        try {
            ...
            if ( ( s != null ) && s.isOpen() ) {
                if ( HibernateUtil.debug ) {
                    HibernateUtil.log.debug( "HIBUTIL.DEBUG_CLOSING_SESSION" ); //$NON-NLS-1$
                }
                s.close();
            }
            HibernateUtil.threadTransaction.set( null );
        } catch ( HibernateException ex ) {
            HibernateUtil.log.error( "HIBUTIL.ERROR_0009_CLOSE_SESSION", ex ); //$NON-NLS-1$
            HibernateUtil.threadTransaction.set( null );
            throw new RepositoryException( "HIBUTIL.ERROR_0009_CLOSE_SESSION", ex ); //$NON-NLS-1$
        }
    ...
    public static void beginTransaction() throws RepositoryException {
        ...
        try {
            if ( tx == null ) {
                if ( HibernateUtil.debug ) {
                    HibernateUtil.log.debug( "HIBUTIL.DEBUG_START_TRANS" ); //$NON-NLS-1$
                }
                tx = HibernateUtil.getSession().beginTransaction();
                HibernateUtil.threadTransaction.set( tx );
            }
        } catch ( HibernateException ex ) {
            HibernateUtil.log.error( "HIBUTIL.ERROR_0004_START_TRANS", ex ); //$NON-NLS-1$
            throw new RepositoryException( "HIBUTIL.ERROR_0004_START_TRANS", ex ); //$NON-NLS-1$
        }
    }
    ...
    public static void commitTransaction() throws RepositoryException {
        ...
        try {
            if ( ( tx != null ) && !tx.wasCommitted() && !tx.wasRolledBack() ) {
                if ( HibernateUtil.debug ) {
                    HibernateUtil.log.debug( "HIBUTIL.DEBUG_COMMIT_TRANS" ); //$NON-NLS-1$
                }
                tx.commit();
            }
        } catch ( HibernateException ex ) {
            HibernateUtil.log.error( "HIBUTIL.ERROR_0008_COMMIT_TRANS", ex ); //$NON-NLS-1$
            try {
            ...
            if ( ex instanceof ConstraintViolationException ) {
                throw new RepositoryException( "HIBUTIL.ERROR_0008_COMMIT_TRANS", ex ); //$NON-NLS-1$
            }
            // throw new
    ...
    public static void rollbackTransaction() throws RepositoryException {
        ...
        try {
            ...
            if ( ( tx != null ) && !tx.wasCommitted() && !tx.wasRolledBack() ) {
                if ( HibernateUtil.debug ) {
                    HibernateUtil.log.debug( "HIBUTIL.DEBUG_ROLLBACK"  ); //$NON-NLS-1$
                }
                tx.rollback();
            }
        } catch ( HibernateException ex ) {
            HibernateUtil.log.error( "HIBUTIL.ERROR_0003_ROLLBACK", ex ); //$NON-NLS-1$
            throw new RepositoryException( "HIBUTIL.ERROR_0003_ROLLBACK", ex ); //$NON-NLS-1$
        } finally {
   ...
   */
    ...
    public static Session disconnectSession() throws RepositoryException {
        ...
        try {
            ...
            HibernateUtil.log.error( "HIBUTIL.ERROR_0002_DISCONNECT", ex ); //$NON-NLS-1$
            throw new RepositoryException( "HIBUTIL.ERROR_0002_DISCONNECT", ex ); //$NON-NLS-1$
        }
        return session;
    }
    ...
    private static Interceptor getInterceptor() {
        return (Interceptor) HibernateUtil.threadInterceptor.get();
    }
    ...
    public static void makePersistent( final Object obj ) throws RepositoryException {
        if ( HibernateUtil.debug ) {
            HibernateUtil.log.debug( "HIBUTIL.DEBUG_MAKE_PERSISTENT" + obj.toString()  ); //$NON-NLS-1$
        }
        try {
            HibernateUtil.getSession().saveOrUpdate( obj );
        } catch ( HibernateException ex ) {
            HibernateUtil.log.error( "HIBUTIL.ERROR_0010_SAVING_UPDATING", ex ); //$NON-NLS-1$
            throw new ContentException( "HIBUTIL.ERROR_0010_SAVING_UPDATING", ex ); //$NON-NLS-1$
        }
    }
    ...
    public static void makeTransient( final Object obj ) throws RepositoryException {
        if ( HibernateUtil.debug ) {
            HibernateUtil.log.debug( "HIBUTIL.DEBUG_MAKE_TRANSIENT" + obj.toString()  ); //$NON-NLS-1$
        }
        try {
            HibernateUtil.getSession().delete( obj );
        } catch ( HibernateException ex ) {
            HibernateUtil.log.error( "HIBUTIL.ERROR_0011_DELETING_OBJ", ex ); //$NON-NLS-1$
            throw new ContentException( "HIBUTIL.ERROR_0011_DELETING_OBJ", ex ); //$NON-NLS-1$
        }
    }
    ...
    public static void evict( final Object obj ) {
        ...
        try {
            ...
            HibernateUtil.log.error( "HIBUTIL.ERROR_0014_EVICTING_OBJECT", ex ); //$NON-NLS-1$
        }
